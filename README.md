...
- Cassandra data за 30 дней: 54 + 2.3 + 1.4 + 0.238 + 0.198 + 0.230 + 0.394 = ~58.8 ТБ
- Cassandra index: 5 + 0.432 + 0.432 + 0.029 + 0.029 + 0.043 + 0.035 = ~6.0 ТБ
- Cassandra RF: 3
...
| Компонент | Пиковый RPS | CPU (vCPU) | RAM (GB) | Disk | Количество | Формула |
|----------|------------:|-----------:|---------:|------|-----------:|--------|
| Chat сервис | 208,000 | 60 | 120 | - | 30 | ceil(208000/3500)=60 |
| Signaling сервис | 34,800 | 10 | 20 | - | 9 | ceil(34800/3500)=10 |
| API сервисы | 20,800 | 6 | 12 | - | 9 | ceil(20800/3500)=6 |
| Kafka | - | 48 | 192 | 6 TB NVMe | 3 | RF=3 |
| Redis sessions | - | 16 | 128 | - | 2 | master+replica |
| PostgreSQL users | - | 32 | 128 | 4 TB NVMe | 2 | primary+replica |
| Cassandra (встречи+чат) | - | 576 | 2304 | 230 TB NVMe raw | 9 | disk расчет ниже |
| Worker: денормализация | - | 16 | 64 | - | 2 | N+1 |
| Worker: уведомления | - | 16 | 64 | - | 2 | N+1 |
| Worker: обработка записи | - | 32 | 128 | - | 2 | N+1 |
| Логи и метрики | - | 16 | 64 | 4 TB | 2 | replica |
| NGINX L7 | - | - | - | - | 72 | раздел 4.2 |
| L4 балансировщик | - | - | - | - | 6 | active-active |

### Диск Cassandra

- данные + индекс: 58.8 + 6.0 = 64.8 ТБ
- с RF=3: 64.8 * 3 = 194.4 ТБ
- с запасом 30 процентов: 194.4 * 1.3 = ~252.7 ТБ

При конфигурации ноды 8 * 3.84 TB NVMe (30.7 TB raw) нужно:

- 252.7 / 30.7 = 8.2 нод, округляем до 9 нод

### Диск PostgreSQL users

- data + index: 110 + 20 = 130 ГБ
- primary + replica: 130 * 2 = 260 ГБ
- с запасом 30 процентов: 260 * 1.3 = ~338 ГБ
...
- Cassandra берем AX162, цена 272.51 EUR в месяц [Hetzner AX Matrix]
- NGINX, L4, workers, Kafka, PostgreSQL, Redis, логи берем EX44, цена 52.36 EUR в месяц [Hetzner EX44]
- Cassandra: 9 * 272.51 = 2452.59 EUR в месяц
- Остальное: (72 + 6 + 6 + 3 + 2 + 2 + 2) * 52.36 = 93 * 52.36 = 4869.48 EUR в месяц
- Итого: 2452.59 + 4869.48 = 7322.07 EUR в месяц
- за 60 месяцев: 7322.07 * 60 = 439324.2 EUR
...
Итоговый выбор: аренда выделенных серверов Hetzner, ориентировочная стоимость 7322.07 EUR в месяц, за 5 лет 439324.2 EUR
