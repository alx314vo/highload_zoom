...
1. Клиент каждую секунду отправляет метрики сети
...
5. По итогу звонок не обрывается, а качество меняется ступенчато
...
graph TB
    ...
    API -.-> MQ[Kafka async]
    MQ -.-> W1[Worker: денормализация async]
    MQ -.-> W2[Worker: уведомления async]
    MQ -.-> W3[Worker: обработка записи async]
...
| Логи и метрики | отдельный контур хранения, отказ одного узла не останавливает сбор |

## **11. Выбор оборудования и хостинга**

### **11.1 Удельное потребление ресурсов**

| Технология | Тип нагрузки | RPS/ядро | RAM/экземпляр |
|------------|--------------|---------:|--------------:|
| Go | легкий | 3,500 | 100 MB |
| Go | средний | 1,000 | 200 MB |
| Nginx | легкий (SSL) | 500 CPS | 10 MB |
| PostgreSQL | средний | 1 | 1 GB |
| Cassandra | средний | 1,000 | 2 GB |

### **11.2 Тип нагрузки по сервисам**

| Сервис | Тип нагрузки | Обоснование |
|--------|--------------|-------------|
| Chat | легкий | JSON API, запись в БД |
| Signaling | легкий | JSON API, WebSocket |
| API | средний | авторизация, бизнес-логика |
| NGINX L7 | легкий | проксирование, TLS |
| Workers | средний | обработка событий |

### **11.3 Распределение нагрузки по ДЦ**

| ДЦ | Доля | MAU | Пользователи |
|----|-----:|----:|-------------:|
| США | 45% | 225M | Северная и Южная Америка |
| Европа | 30% | 150M | Европа, Ближний Восток |
| Азия | 25% | 125M | Азия, Океания, Африка |

### **11.4 Stateless сервисы (Kubernetes)**

**Формулы:**
- CPU = RPS_пик / RPS_на_ядро
- RAM = CPU × 0.5 GB (для лёгких), CPU × 1 GB (для средних)

| Сервис | Пик RPS (всего) | США (45%) | Европа (30%) | Азия (25%) | CPU (всего) | RAM (всего) |
|--------|----------------:|----------:|-------------:|-----------:|------------:|------------:|
| Chat | 208,000 | 93,600 | 62,400 | 52,000 | 60 vCPU | 30 GB |
| Signaling | 34,800 | 15,660 | 10,440 | 8,700 | 10 vCPU | 5 GB |
| API | 20,800 | 9,360 | 6,240 | 5,200 | 21 vCPU | 21 GB |
| Workers | - | - | - | - | 48 vCPU | 48 GB |
| Мониторинг | - | - | - | - | 8 vCPU | 16 GB |
| **Итого** | - | - | - | - | **147 vCPU** | **120 GB** |

**По ДЦ:**

| ДЦ | CPU | RAM | Нод (14 cores, 64 GB) |
|----|----:|----:|----------------------:|
| США | 66 vCPU | 54 GB | 5 |
| Европа | 44 vCPU | 36 GB | 4 |
| Азия | 37 vCPU | 30 GB | 3 |
| **Всего** | **147 vCPU** | **120 GB** | **12** |

### **11.5 NGINX L7**

| Метрика | Всего | США | Европа | Азия |
|---------|-----:|----:|-------:|-----:|
| CPS (пик) | 90,000 | 40,500 | 27,000 | 22,500 |
| CPU | 180 vCPU | 81 | 54 | 45 |
| RAM | 36 GB | 16 GB | 11 GB | 9 GB |
| Инстансов | 90 | 41 | 27 | 22 |

### **11.6 База данных: PostgreSQL (users)**

**Нагрузка:**
- QPS чтение: 10,000 (пик)
- QPS запись: 1,000
- Данные: 110 GB
- Индекс: 20 GB

**Ресурсы:**
- CPU: 10,000 / 1 = 10 cores (чтение) + 2 cores (запись) = 12 cores
- RAM: 12 × 1 GB = 12 GB (кэш + буферы)
- SSD: (110 + 20) × 2 (primary + replica) × 1.3 = 338 GB

**По ДЦ:**

| ДЦ | CPU | RAM | SSD | Инстансов |
|----|----:|----:|----:|----------:|
| США | 6 cores | 6 GB | 170 GB | 2 |
| Европа | 4 cores | 4 GB | 113 GB | 2 |
| Азия | 3 cores | 3 GB | 85 GB | 2 |
| **Всего** | **13 cores** | **13 GB** | **368 GB** | **6** |

### **11.7 База данных: Cassandra**

**Нагрузка:**
- QPS запись: 700,000
- Данные за 30 дней: 58.8 TB
- Индекс: 6.0 TB
- RF: 3

**Ресурсы:**
- CPU: 700,000 / 1,000 = 700 cores (чтение + запись)
- RAM: 700 × 2 GB = 1,400 GB (кэш row cache, key cache)
- SSD: (58.8 + 6.0) × 3 (RF) × 1.3 = 253 TB

**По ДЦ:**

| ДЦ | CPU | RAM | SSD | Нод (32 cores, 256 GB) |
|----|-----:|-----:|-----:|------------------------:|
| США | 315 cores | 630 GB | 114 TB | 12 |
| Европа | 210 cores | 420 GB | 76 TB | 8 |
| Азия | 175 cores | 350 GB | 63 TB | 7 |
| **Всего** | **700 cores** | **1,400 GB** | **253 TB** | **27** |

### **11.8 База данных: Redis (sessions)**
...
**Ресурсы:**
- CPU: 50,000 / 3,500 = 15 cores
- RAM: 30 GB × 2 (master + replica) × 1.3 = 78 GB
- SSD: 20 GB (AOF persistence)

**По ДЦ:**

| ДЦ | CPU | RAM | SSD | Инстансов |
|----|----:|----:|----:|----------:|
| США | 7 cores | 35 GB | 9 GB | 2 |
| Европа | 5 cores | 23 GB | 6 GB | 2 |
| Азия | 4 cores | 20 GB | 5 GB | 2 |
| **Всего** | **16 cores** | **78 GB** | **20 GB** | **6** |
...
- msg/s: 200,000
- Retention: 7 дней
- Данные: ~50 GB/день × 7 = 350 GB

**Ресурсы:**
- CPU: 200,000 / 10,000 = 20 cores (compression, replication)
- RAM: 20 × 4 GB = 80 GB (page cache)
- SSD: 350 GB × 3 (RF) × 1.3 = 1,365 GB

**По ДЦ:**

| ДЦ | CPU | RAM | SSD | Брокеров |
|----|----:|----:|-----:|---------:|
| США | 9 cores | 36 GB | 614 GB | 3 |
| Европа | 6 cores | 24 GB | 409 GB | 3 |
| Азия | 5 cores | 20 GB | 341 GB | 3 |
| **Всего** | **20 cores** | **80 GB** | **1,364 GB** | **9** |

### **11.10 Object Storage (S3): записи встреч**

**Нагрузка:**
- Записей в день: 6,000,000
- Размер записи: 350 MB
- Рост в день: 6M × 350 MB = 2,100 TB

**т к ресурс со сроком хранения 365 дней:**
- Объём: 2,100 TB × 365 = 766,500 TB = 766 PB
- Цена S3: ~$0.02/GB/мес

**По ДЦ:**

| ДЦ | Объём | Стоимость/мес |
|----|------:|-------------:|
| США | 345 PB | $7.0M |
| Европа | 230 PB | $4.6M |
| Азия | 192 PB | $3.9M |
...
### **11.11 Итоговая таблица ресурсов**

| Компонент | CPU | RAM | SSD/S3 | Кол-во нод |
|-----------|----:|----:|-------:|----------:|
| Kubernetes workers | 147 vCPU | 120 GB | - | 12 |
| NGINX L7 | 180 vCPU | 36 GB | - | 90 |
| PostgreSQL | 13 cores | 13 GB | 368 GB | 6 |
| Cassandra | 700 cores | 1,400 GB | 253 TB | 27 |
| Redis | 16 cores | 78 GB | 20 GB | 6 |
| Kafka | 20 cores | 80 GB | 1.4 TB | 9 |
| S3 (записи) | - | - | 766 PB | managed |

### **11.12 Конфигурации серверов**

| Название | CPU | RAM | SSD | Цена/мес |
|----------|----:|----:|----:|---------:|
| kubenode (EX44) | 14 cores | 64 GB | 1 TB | €52 |
| cassandra (AX162) | 48 cores | 256 GB | 15 TB | €273 |
| db-node (AX42) | 48 cores | 128 GB | 4 TB | €141 |

### **11.13 Стоимость (аренда Hetzner)**

| Компонент | Модель | Кол-во | Цена/мес | Итого/мес |
|----------|--------|-------:|---------:|----------:|
| Kubernetes | EX44 | 12 | €52 | €624 |
| NGINX L7 | EX44 | 90 | €52 | €4,680 |
| Cassandra | AX162 | 27 | €273 | €7,371 |
| PostgreSQL | EX44 | 6 | €52 | €312 |
| Redis | EX44 | 6 | €52 | €312 |
| Kafka | AX42 | 9 | €141 | €1,269 |
| L4 балансировщик | EX44 | 9 | €52 | €468 |
| Мониторинг | EX44 | 6 | €52 | €312 |
| **Compute Итого** | - | 165 | - | **€15,348** |
| **S3 (записи)** | - | - | - | **$15.4M** |
| **Всего/мес** | - | - | - | **~$15.5M** |

**За 5 лет (compute):** €15,348 × 60 = **€920,880**

**За 5 лет (S3):** $15.4M × 60 = **$924M**

### **11.14 Сравнение моделей хостинга**

#### Вариант 1: Аренда Hetzner
...
**За 5 лет:** €15,348 × 60 = **€920,880**

#### Вариант 2: Покупка серверов (Dell PowerEdge R760)

Амортизация: 20% в год, за 5 лет — 100% стоимости.

| Группа | Кол-во | Цена/ед. | Итого |
|--------|-------:|---------:|------:|
| Cassandra | 27 | $44,432 | $1,199,664 |
| Kubernetes workers | 12 | $10,899 | $130,788 |
| NGINX L7 | 90 | $10,899 | $980,910 |
| Kafka | 9 | $15,000 | $135,000 |
| DB (PostgreSQL, Redis) | 12 | $10,899 | $130,788 |
| L4 + Мониторинг | 15 | $10,899 | $163,485 |
| **CAPEX** | 165 | - | **$2,740,635** |


#### Вариант 3: Облако

| Ресурс | Кол-во | Цена | Итого/мес |
|--------|-------:|-----:|----------:|
| vCPU | 500 | $20/мес | $10,000 |
| RAM | 1,000 GB | $10/GB/мес | $10,000 |
| SSD (Cassandra) | 253 TB | $0.10/GB/мес | $25,300 |
| **Compute Итого** | - | - | **$45,300** |

**За 5 лет:** $45,300 × 60 = **$2,718,000**

#### Сводная таблица (compute, без S3)

| Вариант | Стоимость/мес | За 5 лет |
|--------|-------------:|---------:|
| Аренда Hetzner | €15,348 (~$16,500) | €920,880 (~$990,000) |
| Покупка + colocation | $65,510 | $3,930,635 |
| Облако | $45,300 | $2,718,000 |

**S3 (записи) — только cloud:**
- Объём: 766 PB
- Стоимость/мес: $15.4M
- За 5 лет: **$924M**

### **11.15 Итоговое решение**

| Компонент | Модель | Обоснование |
|-----------|--------|-------------|
| Compute | Аренда Hetzner | Дешевле облака в 3 раза |
| S3 (записи) | AWS S3 / GCS | Только cloud |

**Итоговая стоимость:**
- Compute: €920,880 за 5 лет
- S3: $924M за 5 лет
...
[Hetzner EX44] - цена сервера EX44
