...
erDiagram
    ...
    users ||--o{ meeting_participant_sessions : "has"
    users ||--o{ chat_messages : "writes"
    users ||--o{ user_sessions : "has"

    meetings ||--o{ meeting_participants : "has"
    meetings ||--o{ meeting_invite_links : "has"
    meetings ||--o{ meeting_participant_sessions : "has"
...
| `meeting_participants` | Текущие участники встречи и их роль |
| `meeting_invite_links` | Инвайт-ссылки для приглашений на встречу |
| `meeting_participant_sessions` | Интервалы участия: joined_at и left_at для повторных подключений |
...
Для каждого подключения хранится свой интервал: `left_at` может быть пустым, если участник сейчас на встрече
...
graph TB
    subgraph "PostgreSQL"
        PG[users, meetings, meeting_invite_links, recordings_metadata]
    end

    subgraph "Cassandra"
        CASS[chat_messages, meeting_participant_sessions]
    end
...
|---|---|---|
| `users`, `meetings`, `meeting_invite_links`, `recordings` | PostgreSQL | Нужны связи и строгие ограничения для метаданных и токенов |
| `chat_messages`, `meeting_participant_sessions` | Cassandra | Много записей, низкая задержка записи в регионе, чтение по `meeting_id` |
...
| `meeting_participant_sessions` | partition `meeting_id`, сортировка по `joined_at` | таймлайн подключений |

### **Шардирование**

| Таблица | Подход | Почему шардируем |
|---|---|---|
| `meetings` | шардирование по `meeting_id` | распределяем нагрузку на метаданные между узлами и уменьшаем горячие точки |
| `meeting_invite_links` | шардирование по `meeting_id` | токеновые проверки привязаны к конкретной встрече |

### **Партиционирование**

| Таблица | Партиционирование | Почему партиционируем |
|---|---|---|
| `chat_messages` | partition key `meeting_id` | запросы чаще всего “по одной встрече” |
| `meeting_participant_sessions` | partition key `meeting_id` | интервалы участия нужны внутри одной встречи |
