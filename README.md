...
Расчет ведется от пиковой нагрузки (раздел 2) и объемов данных/индексов (раздел 6)

Формула для Go сервисов:

CPU(vCPU) = ceil(RPS_peak / 3500)

Диск для Cassandra:

Disk = (Data + Index) * RF * 1.3

### **11.2 Входные цифры**

- количество ДЦ: 3
- chat QPS (пик): 208,000
- signaling QPS (пик): 34,800
- API join QPS (пик): 20,800
- производительность Go: 3,500 RPS на 1 vCPU
- Cassandra data за 30 дней: ~58.8 ТБ
- Cassandra index: ~6.0 ТБ
- Cassandra RF: 3
- disk запас: 30 процентов
- PostgreSQL users data: ~110 ГБ
- PostgreSQL users index(email): ~20 ГБ

### **11.3 Расчет ресурсов по сервисам (на 1 ДЦ)**

| Сервис | Пиковый RPS | CPU(vCPU) | RAM(GB) | Кол-во инстансов | Расчет |
|--------|------------:|----------:|--------:|-----------------:|--------|
| Chat сервис | 208,000 | 60 | 120 | 30 | ceil(208000/3500)=60, 2 инстанса на 1 vCPU |
| Signaling сервис | 34,800 | 10 | 20 | 9 | ceil(34800/3500)=10, 1 vCPU на инстанс |
| API сервисы | 20,800 | 6 | 12 | 9 | ceil(20800/3500)=6, 1 vCPU на инстанс |
| Worker: денормализация | - | 16 | 64 | 2 | N+1 |
| Worker: уведомления | - | 16 | 64 | 2 | N+1 |
| Worker: обработка записи | - | 32 | 128 | 2 | N+1 |
| Логи и метрики | - | 16 | 64 | 2 | replica |

Итого на 1 ДЦ по stateless:

- CPU: 60 + 10 + 6 + 16 + 16 + 32 + 16 = 156 vCPU
- RAM: 120 + 20 + 12 + 64 + 64 + 128 + 64 = 472 GB

Kubernetes worker nodes (на 1 ДЦ):

- берем ноду EX44: 64 GB RAM, 14 CPU cores [Hetzner EX44]
- нужно по RAM: 472 / 64 = 7.4, округляем до 8 нод
- нужно по CPU: 156 / 14 = 11.1, округляем до 12 нод
- итог: 12 нод на 1 ДЦ, 36 нод на 3 ДЦ

### **11.4 Stateful компоненты**

#### Cassandra (диск)

- данные + индекс: 58.8 + 6.0 = 64.8 ТБ
- Disk = 64.8 * 3 * 1.3 = ~252.7 ТБ на 1 ДЦ
- при 30.7 ТБ raw на ноду: 252.7 / 30.7 = 8.2, берем 9 нод на 1 ДЦ
- по 3 ДЦ: 27 нод

#### PostgreSQL users (диск)

- data + index: 110 + 20 = 130 ГБ
- primary + replica: 130 * 2 = 260 ГБ
- с запасом: 260 * 1.3 = ~338 ГБ на ДЦ

### **11.5 Выбор хостинга**

Модель: гибридная

- stateless сервисы (Go, workers, мониторинг): VM или Kubernetes
- stateful (Cassandra, Kafka, Redis, PostgreSQL): bare-metal

### **11.6 Стоимость на 5 лет**
...
- AX162: 272.51 EUR в месяц [Hetzner AX Matrix]
- EX44: 52.36 EUR в месяц [Hetzner EX44]

| Компонент | Модель | Кол-во | Цена/мес | Итого/мес |
|----------|--------|-------:|---------:|----------:|
| Cassandra (3 ДЦ) | AX162 | 27 | 272.51 | 7357.77 |
| Kubernetes worker nodes (3 ДЦ) | EX44 | 36 | 52.36 | 1884.96 |
| NGINX L7 (3 ДЦ) | EX44 | 216 | 52.36 | 11309.76 |
| L4 балансировщик (3 ДЦ) | EX44 | 18 | 52.36 | 942.48 |
| Kafka (3 ДЦ) | EX44 | 9 | 52.36 | 471.24 |
| Redis (3 ДЦ) | EX44 | 6 | 52.36 | 314.16 |
| PostgreSQL users (3 ДЦ) | EX44 | 6 | 52.36 | 314.16 |
| Логи и метрики (3 ДЦ) | EX44 | 6 | 52.36 | 314.16 |
| **ИТОГО** | - | - | - | **22908.69 EUR/мес** |

За 60 месяцев: 22908.69 * 60 = 1374521.4 EUR

#### Вариант 2: покупка серверов (пример Dell PowerEdge R760)

Амортизация: 20 процентов в год, за 5 лет суммарно 100 процентов стоимости

Берем цену серверов Dell PowerEdge R760 из прайса Dell:

- базовая конфигурация: 10,899 USD [Dell R760]
- более мощная конфигурация: 44,432.34 USD [Dell R760]

Оценка стоимости покупки:

| Группа | Кол-во | Цена | Итого |
|-------|------:|-----:|------:|
| Cassandra (27 серверов) | 27 | 44,432.34 USD | 1,199,673.18 USD |
| Остальные (297 серверов) | 297 | 10,899 USD | 3,237,003 USD |
| **ИТОГО CAPEX** | - | - | **4,436,676.18 USD** |

Амортизация 5 лет по 20 процентов в год: 4,436,676.18 USD
...
- суммарные vCPU (3 ДЦ): Cassandra 576 * 3 = 1,728, stateless 156 * 3 = 468, Kafka 48 * 3 = 144, Redis 16 * 3 = 48, PostgreSQL 32 * 3 = 96, всего 2,484 vCPU
- compute в месяц ≈ 2,484 * 1,459.5 = 3,625,398 руб
- compute за 60 месяцев ≈ 217,523,880 руб

Диск в облаке (только Cassandra, уже с RF и запасом):

- Disk Cassandra total = 252.7 ТБ на ДЦ * 3 ДЦ = 758.1 ТБ = 758,100 ГБ
- по примеру стоимости хранения 1 ТБ в месяц: 1,261 руб [Yandex Object Storage example], значит 1 ГБ в месяц ≈ 1.261 руб
- storage в месяц ≈ 758,100 * 1.261 = 955,964 руб
- storage за 60 месяцев ≈ 57,357,840 руб

Итого облако за 5 лет (compute + storage) ≈ 274,881,720 руб

### Сводная таблица

| Вариант | Что включено | Стоимость в месяц | Стоимость за 5 лет |
|--------|--------------|------------------:|-------------------:|
| Hetzner аренда | AX162 для Cassandra + EX44 для остального (таблица выше) | 22,908.69 EUR | 1,374,521.4 EUR |
| Покупка железа | CAPEX по Dell R760, амортизация 20% в год | - | 4,436,676.18 USD |
| Облако | compute по 2,484 vCPU + storage по 758.1 ТБ | 3,625,398 + 955,964 = 4,581,362 руб | 274,881,720 руб |

Вывод: при больших объемах диска аренда выделенных серверов дешевле облака

Итоговый выбор: аренда выделенных серверов Hetzner, ориентировочная стоимость 22908.69 EUR в месяц, за 5 лет 1374521.4 EUR
