...
Асинхронные паттерны для надежности: ретрай с экспоненциальной задержкой (повторяем упавший запрос с растущими интервалами), Circuit Breaker (размыкаем цепь при серии ошибок, чтобы не долбить мертвый сервис), Backpressure (отбрасываем лишние запросы, чтобы не утонуть в очереди), Transactional Outbox (пишем событие в ту же БД, что и данные и либо всё вместе, либо ничего)

Graceful Degradation: При отказе чата, записи или базы списка встреч пользователь теряет только эти функции, но видеозвонок продолжается без простоя потому что Signaling изолирован от других сервисов

Graceful Shutdown: При обновлении кода сервер отключается от балансировщика, перестаёт принимать новые запросы, доделывает активные звонки и WebSocket-соединения в течение определенного периода, и только после этого завершается

Prometheus вытягивает метрики (CPU, RAM, QPS, Latency) с каждого узла инфраструктуры, а Grafana визуализирует их на дашбордах и сигналит при превышении порогов чтобы можно было оперативно узнать о проблеме
## Источники
